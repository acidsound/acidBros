...
- **Drop Preview Bar** – While dragging a timeline block, a vertical bar appears between blocks to preview the exact insertion point.
- **Edge Glow Preview** – The adjacent blocks at the insertion point also show gradient edge glow so the target remains visible even when your finger covers the bar.
- **Drop Motion Feedback** – After dropping, nearby timeline blocks animate into position; the dragged block only nudges slightly from your drop point into its final slot, then flashes briefly for confirmation.
