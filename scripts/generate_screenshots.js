...
async function main() {
    ...
    try {
        ...
        try {
            ...
            const pianoOverlay = page.locator('.note-editor');
            ...
            console.log('Could not open piano roll: ' + e);
        }
        ...
        // 8. File Manager
        await page.click('#fileManagerBtn');
        await page.waitForSelector('#fileManagerPopover', { state: 'visible' });
        await page.waitForTimeout(500);
        const fileManagerPopover = page.locator('#fileManagerPopover');
        await fileManagerPopover.screenshot({ path: path.join(assetsDir, 'manual-filemanager.png') });
        await page.click('#fileCloseBtn');
        await page.waitForTimeout(500);

        // 9. Song Mode
        await page.click('label[for="mode_song"]');
        await page.waitForTimeout(1000);
        const songModeControls = page.locator('#mode-context-controls');
        await songModeControls.screenshot({ path: path.join(assetsDir, 'manual-songmode.png') });
        await page.click('label[for="mode_pattern"]');
        await page.waitForTimeout(500);

        // 10. Oscilloscope
        const scope = page.locator('.oscilloscope-container');
        await scope.screenshot({ path: path.join(assetsDir, 'manual-oscilloscope.png') });

        // 11. Randomize/Active State
        await page.click('#randomBtn');
        await page.waitForTimeout(1000);
        const tb303Unit1 = page.locator('.machine.tb-303').nth(0);
        await tb303Unit1.screenshot({ path: path.join(assetsDir, 'manual-tb303-active.png') });
